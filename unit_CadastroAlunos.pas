...
Begin
  ...
  if mmFila.Lines.Count < 1 then
    Application.MessageBox('Elemento não foi encontrado', 'Erro', mb_ok + MB_ICONERROR);
end;
...
begin
  ...
  if Selecionado then
    if not Controle.RemoveAluno(Controle.getNoByMatriculaAluno(StrToIntDef(edMatricola.Text,0))) then
      Application.MessageBox(PChar('Não foi localizado nenhum elemento com a Matricula "'+edMatricola.Text+'".'), 'Erro', mb_ok + MB_ICONERROR);
end;
...
begin
  ...
  if (edMatricola.Text = '') then
   begin
        Application.MessageBox('Informe a matrícula!', 'Erro', mb_ok + MB_ICONERROR);
        edMatricola.SetFocus;
        result := false;
        exit;
   end;
